...
def process_pipeline(payload):
    ...
    if not check_rate_limit(gitlab_project_id):
        print(f"Rate limit exceeded for project {gitlab_project_id}, skipping scan")
    ...
    if injection_detected:
        print(f"Prompt injection detected in {injection_file}, pipeline stopped")
    ...
    # Step 5: Gemini security analysis
    try:
    ...
    for finding in report.findings:
        ...
        if hasattr(finding, 'sandbox_template') and finding.sandbox_template:
            ...
            if not template_valid:
                print(f"Sandbox template rejected: {template_reason}")
